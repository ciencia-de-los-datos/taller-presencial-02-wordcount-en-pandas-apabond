...
def count_words(dataframe):
    ...
    dataframe["count"] = 1
    dataframe = dataframe.groupby("text", as_index=False).agg(
        {"count": "sum"}
    )
    #as_index permite que la palabra no quede en la numeracion del dataframe sino que sea en sí una columna
